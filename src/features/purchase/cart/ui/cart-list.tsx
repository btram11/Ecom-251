...
import { CartItemGroup, useCartListContext } from '@entities/cart';
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="container mx-auto px-4 py-6 md:py-8">
        <header className="mb-4 flex flex-col gap-2 md:mb-6 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-xl font-semibold text-slate-900">Giỏ hàng của bạn</h1>
        ...
        {items.length === 0 ? (
          <div className="mx-auto max-w-md rounded-lg border border-dashed border-slate-200 bg-white px-8 py-12 text-center text-sm text-slate-500">
            <p className="font-medium text-slate-600">Chưa có sản phẩm nào trong giỏ.</p>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,3.2fr)_minmax(0,1.4fr)] lg:items-start">
              ...
              <div>
                {groups.map((group) => {
                  const groupAllSelected =
                    group.lines.length > 0 && group.lines.every((it) => it.isSelected);

                  const groupSomeSelected =
                    group.lines.some((it) => it.isSelected) && !groupAllSelected;
            ...
            >
              <h2 className="text-sm font-semibold text-slate-900">Tóm tắt đơn hàng</h2>

              <div className="mt-4 space-y-2 text-sm">
                <div className="flex items-center justify-between text-slate-600">
                  <span>Tạm tính</span>
                  <span>{selectedQty > 0 ? formatCurrency(selectedSubTotal) : '0 đ'}</span>
                </div>
                <div className="flex items-center justify-between text-slate-600">
              ...
              <Button className="mt-4 w-full" disabled={selectedQty === 0} asChild>
