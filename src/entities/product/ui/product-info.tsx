...
export function ProductInfo({ product }: { product: Product }) {
  ...
  return (
    <div className="flex flex-col justify-between">
      ...
      {/* Action Buttons */}
      <div className="flex flex-row gap-x-6 mt-2 text-lg font-bold">
          <button
            onClick={() => alert("Add to cart")}
            className="bg-red-600 text-white rounded-lg p-4 w-64"
          >
            {" "}
            Mua ngay
          </button>
          ...
          <button
            ...
            className="bg-[#00a63e] text-white px-4 py-2.5 w-64 rounded-[10px] hover:bg-[#008c35] transition-colors"
          >
            + Thêm vào giỏ
          </button>
        </div>
